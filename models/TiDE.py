...
class Model(nn.Module):  
    ...
    def __init__(self, configs, bias=True, feature_encode_dim=2): 
        ...
        if self.use_ph:
            self.feature_dim = 1
            flatten_dim = self.seq_len + (self.pred_len) * self.feature_encode_dim
        else:
    ...
    def forecast_topo(self, x_enc, x_mark_enc, x_dec, batch_y_mark, topo_y):
        # Normalization
        means = x_enc.mean(1, keepdim=True).detach()
        x_enc = x_enc - means
        stdev = torch.sqrt(torch.var(x_enc, dim=1, keepdim=True, unbiased=False) + 1e-5)
        ...
        topo_y = topo_y.unsqueeze(-1)

        feature = self.feature_encoder(topo_y)
        hidden = self.encoders(torch.cat([x_enc, feature.reshape(feature.shape[0], -1)], dim=-1))
        decoded = self.decoders(hidden).reshape(hidden.shape[0], self.pred_len, self.decode_dim)

        dec_out = self.temporalDecoder(torch.cat([feature, decoded], dim=-1)).squeeze(-1) + self.residual_proj(x_enc)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, batch_y_mark, topo_y=None, mask=None):
        ...
        if self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast':
            ...
            if self.use_ph:
                dec_out = torch.stack([self.forecast_topo(x_enc[:, :, feature], x_mark_enc, x_dec, batch_y_mark, topo_y=topo_y) for feature in range(x_enc.shape[-1])],dim=-1)
            else:
